...
const mapping = function (data, index) {
  switch (data.type) {
    ...
    case 'dataStructure':
      return SchemaDataStructure(data)
    default:
...
const SchemaDataStructure = data => {
  let dataStructure = yapi.getInst(dataStructureModel).get(data.id);
  if (dataStructure) {
    return this.schemaTransformToTable(dataStructure.toObject());
  }
  return null;
}
